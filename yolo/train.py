...
class DetectionTrainer(BaseTrainer):
    ...
    def get_validator(self):
        ...
        self.loss_names = 'box_loss', 'cls_loss', 'dfl_loss', 'adv_loss', 'dac_loss', 'mlayer_disc_loss'
        return DetectionValidator(self.test_loader, save_dir=self.save_dir, args=copy(self.args))

    def criterion(self, preds, batch, adv_loss=None, d_const_loss=None, mlyrdist_loss=None):
        """Compute loss for YOLO prediction and ground-truth."""
        if not hasattr(self, 'compute_loss'):
            self.compute_loss = Loss(de_parallel(self.model))

        if adv_loss is not None and d_const_loss is not None and mlyrdist_loss is not None:
            return self.compute_loss(preds, batch, adv_loss, d_const_loss, mlyrdist_loss)
        else:
...
class Loss:
    ...
    def __call__(self, preds, batch, adv_loss=None, d_const_loss=None, mlyrdist_loss=None):
